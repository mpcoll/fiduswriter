...
export class ModAnchorStore {
    constructor(mod) {
        mod.anchorStore = this
        this.mod = mod
    ...
    // entirely.
    addAnchor() {
    	let currentAnchor=randomID()    	
    	
        let id ='anchor' +currentAnchor 
        this.mod.editor.pm.doc.firstChild.attrs.topAnchor +=1
        this.anchor = {
            anchor: new Anchor(
                id,
        ...
        this.mod.editor.pm.tr.addMark(this.mod.editor.pm.selection.from, this.mod.editor.pm.selection.to, markType).apply()
    }

    removeAnchor() {
        if (this.anchor) {
            this.mod.editor.pm.removeRange(this.anchor.referrer)
            this.anchor = false
        }
    }
